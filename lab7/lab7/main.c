...
void generate_test_data(LibraryItem* items, int count) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        items[i].pub_year = 1900 + rand() % 120;
        items[i].page_count = 150 + rand() % 500;
    }
}
...
void run_performance_test(int method, int order) {
    const int test_sizes[NUM_TEST_SIZES] = { 100, 1000, 10000, 50000, 100000, 200000, 300000 };
    ...
    for (int i = 0; i < NUM_TEST_SIZES; i++) {
        ...
        default: printf("Invalid method\n"); free(test_set); return;
        }

        clock_t end = clock();
        double elapsed = (double)(end - begin) / CLOCKS_PER_SEC;

        printf("Elements: %7d - Processing time: %.3f seconds\n", current_size, elapsed);
        free(test_set);
    }
}
